...
class JobService {
    ...
    async addList(jobs) {
        if (!Array.isArray(jobs)) return await this.add(jobs);
        if (Array.isArray(jobs) && jobs.length === 1) return await this.add(jobs[0]);
        try {
            jobs.forEach(job => {
                JobService.parseJob(job);
                logger.debug(`add job: ${util.inspect(job)}`);
            });
            let results = await JobDao.save(jobs);
            if (results) {
                results.forEach(async result => {
                    let plain = JobDao.parseDto(result.toJSON());
                    logger.info(`save job: ${util.inspect(plain)}`);
                    // 添加分发定时器
                    await JobService.addToSchedule(plain).catch(err => logger.error(`job ${plain.taskId} add schedule error`, err));
                });
            ...
            logger.error('add jobs error', err);
            throw err;
        }
    }
     ...
     * @returns {Promise.<void>}
     */
    async complete(taskId, msg, hostInfo, success) {
        let reentrantLock = await distributedLockService.lock(taskId, 30000);
        try {
            let job = await JobDao.findOne({taskId, status: Constant.JOB_STATUS.EXECUTE});
            if (!job) throw new Error(`job ${taskId} is not found`);
            await JobLogDao.save({
     ...
     * @returns {Promise.<void>}
     */
    async reset(taskId, msg, hostInfo) {
        let reentrantLock = await distributedLockService.lock(taskId, 30000);
        try {
            let job = await JobDao.findOne({taskId, status: Constant.JOB_STATUS.EXECUTE});
            if (!job) throw new Error(`job ${taskId} is not found`);
            await job.update({status: Constant.JOB_STATUS.RUNNING});
            await JobLogDao.save({
                type: Constant.JOB_LOG_TYPE.RESET,
                taskId: taskId,
                jobId: job.id,
                nodeGroup: job.nodeGroup,
                success: true,
    ...
    static async distribute(taskId) {
        ...
        try {
            ...
            if (![Constant.JOB_STATUS.CREATE, Constant.JOB_STATUS.RUNNING].includes(job.status) && !(job.status === Constant.JOB_STATUS.EXECUTE && job.relyOnPrevCycle === false)) {
            ...
            // 保存任务日志
            let record = {
                type: Constant.JOB_LOG_TYPE.RECEIVE,
                taskId: plain.taskId,
                jobId: plain.id,
            ...
            try {
                ...
                // 更新任务状态为执行中
                await job.update({status: Constant.JOB_STATUS.EXECUTE});
                if (result.msg) record.msg = result.msg;
                record.operationHost = result.info.host;
                record.operationPort = result.info.port;
                record.operationPid = result.info.pid;
            } catch (err) {
