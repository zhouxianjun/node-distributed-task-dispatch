...
class JobService {
    ...
    static async distribute(taskId) {
        ...
        try {
            ...
            if (job.status === Constant.JOB_STATUS.PAUSE) {
                logger.warn(`job: ${job.taskId} is pause`);
                return;
            }
            if (job.status === Constant.JOB_STATUS.CANCEL) {
                logger.warn(`job: ${job.taskId} is cancel`);
                return;
            }
            ...
            switch (job.type) {
                ...
                case Constant.JOB_TYPE.TIMER:
                    if (record.success !== true && job.retryTimes >= job.maxRetryTimes) {
                        await job.update({status: Constant.JOB_STATUS.FAILED, nextTime: Date.now() + (config.retryInterval || 10) * 1000});
                    }
                ...
                case Constant.JOB_TYPE.REPEAT:
                    if (record.success !== true && job.retryTimes >= job.maxRetryTimes && job.repeatedCount >= job.repeatCount) {
                        await job.update({status: Constant.JOB_STATUS.FAILED, nextTime: Date.now() + job.repeatInterval});
                    }
                    if (record.success === true) {
                        await job.update({status: Constant.JOB_STATUS.SUCCESS});
                    }
